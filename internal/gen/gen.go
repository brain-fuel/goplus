...
type Result struct {
	...
	// LocalPkgs are the package paths generated by this run, so a caller
	// can tell its own assumptions from those its dependencies carry.
...
func Run(opts Options) (*Result, error) {
	...
	for _, dir := range dirs {
		...
		if idx != nil && len(diags) == 0 {
			if res.LocalPkgs == nil {
				res.LocalPkgs = map[string]bool{}
			}
			res.LocalPkgs[pkgPath(pkgPathRoot, moduleRoot, dir)] = true

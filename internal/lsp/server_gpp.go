// Code generated by gpp from server.gpp. DO NOT EDIT.
//gpp:v v0.9.0
...
// delegate. Effects called from the gpp dispatch layer live here.
type Server struct {
	conn *conn
	root string // workspace directory

	mu       sync.Mutex
	overlays map[string][]byte         // abs .gpp path → unsaved contents
...
func (s *Server) handle(m *message) (exit bool) {
	...
	case "initialize":
		...
		_ = s.conn.respond(m.ID, initializeResult{
			Capabilities: caps,
			ServerInfo:   serverInfo{Name: "gpp", Version: version.Version},
		})
	case "initialized":
	case "shutdown":
...
func (s *Server) runDiagnostics() {
	...
	if len(res.Diags) == 0 && res.Outputs != nil {
		...
		for genPath, content := range res.Outputs {
			gppPath := strings.TrimSuffix(genPath, "_gpp.go") + ".gpp"
			gppSrc, ok := overlay[gppPath]
			if !ok {
				continue
			}
			s.maps[genPath] = sourcemap.Build(gppPath, gppSrc, content)
		}
		s.mu.Unlock()
		if s.gopls != nil {
...
// Forward maps a .gpp position through the latest generation, asks
...
func (s *Server) Forward(method, uri string, line, ch int) QueryReply {
	...
	gppPath := uriToPath(uri)
	genPath := generatedCounterpart(gppPath)
	s.mu.Lock()
	m := s.maps[genPath]
	s.mu.Unlock()
	if m == nil {
		return NoAnswer{Reason: "no generation for " + gppPath}
	}
...
// their .gpp counterparts, structurally, wherever they appear.
...
func (s *Server) mapBackLocation(uri string, rng map[string]any) (string, map[string]any, bool) {
	...
	return pathToURI(m.GppPath), rng, true
}
...
// generatedCounterpart names the emitted twin of a .gpp file.
func generatedCounterpart(gppPath string) string {
	return strings.TrimSuffix(gppPath, ".gpp") + "_gpp.go"
}

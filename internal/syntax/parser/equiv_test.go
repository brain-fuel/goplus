...
func TestForkEquivalence(t *testing.T) {
	...
	for _, path := range corpus(t) {
		...
		if forkExt != nil && (len(forkExt.Enums) > 0 || len(forkExt.Matches) > 0 ||
			len(forkExt.Classes) > 0 || len(forkExt.Instances) > 0 || len(forkExt.Tails) > 0 || len(forkExt.Refinements) > 0) {
			t.Errorf("%s: pure Go produced extensions: %d enums, %d matches, %d classes, %d instances, %d tails, %d refinements",
				path, len(forkExt.Enums), len(forkExt.Matches), len(forkExt.Classes), len(forkExt.Instances), len(forkExt.Tails), len(forkExt.Refinements))
		}

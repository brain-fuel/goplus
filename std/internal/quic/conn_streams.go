...
func (c *Conn) streamsCleanup() {
	...
	c.streams.streamsMu.RLock()
	streams := make([]maybeStream, 0, len(c.streams.streams))
	for _, s := range c.streams.streams {
		streams = append(streams, s)
	}
	c.streams.streamsMu.RUnlock()
...
func (c *Conn) newLocalStream(ctx context.Context, styp streamType) (*Stream, error) {
	...
	// The stream registry is independently synchronized, so opening a request
	// stream doesn't need a round trip through the connection event loop.
	c.setStream(s.id, maybeStream{s})
	return s, nil
}

func (c *Conn) getStream(id streamID) (maybeStream, bool) {
	c.streams.streamsMu.RLock()
	stream, ok := c.streams.streams[id]
	c.streams.streamsMu.RUnlock()
	return stream, ok
}

func (c *Conn) setStream(id streamID, stream maybeStream) {
	c.streams.streamsMu.Lock()
	c.streams.streams[id] = stream
	c.streams.streamsMu.Unlock()
}

func (c *Conn) deleteStream(id streamID) {
	c.streams.streamsMu.Lock()
	delete(c.streams.streams, id)
	c.streams.streamsMu.Unlock()
}
...
func (c *Conn) streamForFrame(now time.Time, id streamID, ftype streamFrameType) (*Stream, bool) {
	...
	c.setStream(id, maybeStream{s})
	return s, true
}

// publishStream makes a newly peer-created stream visible to AcceptStream.
// Callers do this only after applying the frame that created the stream, so an
// awakened reader observes the initial data or terminal state immediately.
func (c *Conn) publishStream(s *Stream, created bool) {
	if created {

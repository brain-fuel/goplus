...
func (c *netUDPConn) Write(dgram datagram) error {
	requestedIP := dgram.localAddr.Addr()
	if dgram.ecn == ecnNotECT && c.localAddr.IsValid() &&
		(!requestedIP.IsValid() || requestedIP == c.localAddr.Addr()) {
		// A socket bound to a concrete address already has the requested
		// source. Avoid sendmsg and ancillary source-address data on this
		// common server path.

...
type Record struct { ID string; Kind string; Next int; Done bool }
...
func Run(ctx context.Context, journal Journal, saga Saga) error {
	...
	if !found { record = Record{ID: saga.ID, Kind: saga.Kind}; if err := journal.Save(ctx, record); err != nil { return err } }
...
func Compensate(ctx context.Context, journal Journal, saga Saga) error {
	record, found, err := journal.Load(ctx, saga.ID); if err != nil { return err }; if !found { return nil }
	for i := record.Next - 1; i >= 0; i-- { if saga.Steps[i].Compensate != nil { if err := saga.Steps[i].Compensate(ctx); err != nil { return fmt.Errorf("workflow %s compensate %s: %w", saga.ID, saga.Steps[i].Name, err) } } }

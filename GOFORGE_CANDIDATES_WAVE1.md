...
| goml goal spelling | — | v0.147.1 | std/v0.212.0 | — | brain-fuel/goplus (`goforge.dev/goplus`, `cmd/goml`) | `/goplus/` published | hole goals re-spelled into goml notation (`Vec a (n + 1)`, `Slice String`, `Int -> String`); dependent instantiations split textually since Go rejects index terms; Go→goml names inverted from gomlBuiltins so the directions cannot drift; 433 scenarios, race + gen -check clean; released 2026-08-27 |
| goml in the LSP | — | v0.148.0 | std/v0.212.0 | — | brain-fuel/goplus (`goforge.dev/goplus`, `goplus lsp`) | `/goplus/` published | `goplus lsp` serves `.goml` buffers (in-memory transpile via goml.Run overlay support); hole goals hover in goml spelling; delegated hover/definition stay .gp-only (no two-hop map); fixed nvim/zed clients keying on a `.goplus` suffix so they never activated; 433 scenarios, race + gen -check clean; released 2026-08-27 |
| assume (Stage B opens) | — | v0.149.0 | std/v0.212.0 | — | brain-fuel/goplus (`goforge.dev/goplus`) | `/goplus/` published | proof argument may be `assume` (asserted) as well as `refl` (proved); erases identically; every use recorded and listed by `goplus assumptions`; recorded on the observing pass (the audit pass no longer sees the erased argument) and position anchored on the token (the line diff cannot anchor a lowered call); 439 scenarios, race + gen -check clean; released 2026-08-28 |
| assumption markers | — | v0.150.0 | std/v0.212.0 | — | brain-fuel/goplus (`goforge.dev/goplus`) | `/goplus/` published | `//goplus:assume` carries an assumption into the distributed artifact so a consumer can audit what its dependencies assumed; marker omits source position to keep `gen -check` stable; `goplus assumptions` separates local (positioned) from dependency (by package) via gen.Result.LocalPkgs; verified across two modules; race + gen -check clean; released 2026-08-28 |
| ordering propositions | — | v0.151.0 | std/v0.212.0 | — | brain-fuel/goplus (`goforge.dev/goplus`) | `/goplus/` published | `Le`/`Lt` join `Eq` as statable propositions, so a bound is expressible; decider needed no new power (FactGe already existed) — only a shared `core.PropOp` replacing four string comparisons; `decide` is the general witness, `refl` stays Eq-only because it means reflexivity; propositions not yet hypotheses (documented); race + gen -check clean; released 2026-08-28 |
| hypotheses + forwarding | — | v0.152.0 | std/v0.212.0 | — | brain-fuel/goplus (`goforge.dev/goplus`) | `/goplus/` published | a proposition in scope is a hypothesis (Decide always took hyps; the proof path passed nil), and an erased index may be forwarded as a call argument — without which hypotheses were unreachable; misuse still refused, with a message explaining erasure. FOUND (unfixed, documented): omitting a proof argument skips its obligation, predating v0.146.0; race + gen -check clean; released 2026-08-28 |
| proof soundness | — | v0.153.0 | std/v0.212.0 | — | brain-fuel/goplus (`goforge.dev/goplus`) | `/goplus/` published | FIXES a bypass predating v0.146.0: omitting a proof argument skipped its obligation, because an omitted call is textually identical to an erased one. Obligations now settle on resolve iteration 0, before erasure exists, with a type-info-free syntactic check (a match arm's binders do not exist that early). Proof-carrying functions may only be used in a direct call (composition/pipe/partial/assignment all bypassed). std regenerates byte-identically; race + gen -check clean; released 2026-08-29 |
| match refinement | — | v0.154.0 | std/v0.212.0 | — | brain-fuel/goplus (`goforge.dev/goplus`) | `/goplus/` published | a proposition in scope prunes a variant its bound excludes (`Lt[0, n]` rules out `Nil : Vec[T, 0]`); decider needed no new power a third time — impossibility was sign analysis, now falls back to a strict inequality under hypotheses; generated boundary guard agrees so plain-Go callers panic by name; stays conservative (`Le[0, n]` does not prune); std byte-identical, race + gen -check clean; released 2026-08-29 |
| conjunction | — | v0.155.0 | std/v0.212.0 | — | brain-fuel/goplus (`goforge.dev/goplus`) | `/goplus/` published | `And[P, Q]` whose arguments are propositions, so the whole precondition (`0 <= i` and `i < n`) fits one proof parameter; costs the decider nothing since Decide already takes a list of facts; nests, mixes relations, and inherits erasure/witnesses/audit/match-refinement unchanged; diagnostics substitute call arguments into each part; std byte-identical, race + gen -check clean; released 2026-08-29 |
| named propositions (Stage B complete) | — | v0.156.0 | std/v0.212.0 | — | brain-fuel/goplus (`goforge.dev/goplus`) | `/goplus/` published | `type InRange[i nat, n nat] prop { … }` names a proposition; a name is an ABBREVIATION so a use unfolds and everything downstream (discharge, hypotheses, match refinement, boundary guard, assume audit) works unchanged; declaration erases completely to a free-floating `//goplus:prop` marker so consumers can unfold it; props may name props, unfolding bounded against self-reference; contextual `prop` joins enum/class/refine at one hook. COMPLETES GOALS.md Stage B; std byte-identical, race + gen -check clean; released 2026-08-29 |
| std/vec proposition consumer | — | v0.156.0 | std/v0.213.0 | — | brain-fuel/goplus (`goforge.dev/goplus/std/vec`) | `/goplus/` published | Stage B's first real consumer: `AtIndex`/`Set` take a plain number with the bound as `Lt[i, n]` instead of `Fin[n]`, so it erases (generated `func AtIndex[T any](i int, values Vec[T]) T` + boundary guard) and an out-of-range index is a compile error. Exercises the whole stage: precondition at the call, hypothesis pruning the Nil arm, and the fact discharging `Lt[i-1, n-1]`. Cross-module scenario pins the rejection; std tests + gen -check clean; released 2026-08-29 |
